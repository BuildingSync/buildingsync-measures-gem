class ReplaceWithGroundSourceHeatPumpHvac < OpenStudio::Measure::ModelMeasure
  def name = 'Replace HVAC with Ground-Source Heat Pumps'
  def description = 'Replaces HVAC dedicated to selected zones with ground-coupled water-to-air heat pumps.'
  def modeler_description = 'Rejects partial shared-loop removal, preserves existing plants, and creates Ground Source Heat Pumps.'
  ...
  def run(model, runner, user_arguments)
    ...
    begin
      ...
      unless unserved.empty?
        runner.registerError("Ground-source heat-pump replacement did not serve: #{unserved.map { |zone| zone.name.to_s }.join(', ')}")
        return false
      end
      ...
      runner.registerError("Ground-source heat-pump replacement failed: #{e.message}")
      false
    end
  end
end

ReplaceWithGroundSourceHeatPumpHvac.new.registerWithApplication
