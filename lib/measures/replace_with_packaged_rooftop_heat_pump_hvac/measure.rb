class ReplaceWithPackagedRooftopHeatPumpHvac < OpenStudio::Measure::ModelMeasure
  def name
    'Replace HVAC with Packaged Rooftop Heat Pump'
  end

  def description
    'Replaces HVAC dedicated to selected zones with packaged single-zone rooftop heat pumps.'
  end

  def modeler_description
    'Rejects partial shared-loop removal, removes only selected-zone equipment and dedicated loops, and creates PSZ-HP systems.'
  end
  ...
  def arguments(_model)
    ...
    template.setDescription('openstudio-standards template used to construct replacement PSZ-HP systems.')
  ...
  def run(model, runner, user_arguments)
    ...
    begin
      ...
      unless missing_zones.empty?
        runner.registerError("PSZ-HP replacement did not serve: #{missing_zones.map { |zone| zone.name.to_s }.join(', ')}")
        return false
      end
      runner.registerFinalCondition(
        "Removed #{affected.size} dedicated air loops and #{removed_equipment} zone HVAC objects; " \
        "created #{created.size} PSZ-HP loops for #{zones.size} zones. Existing plant loops were preserved."
      )
      true
    rescue LoadError => e
      ...
      runner.registerError("PSZ-HP replacement failed: #{e.message}")
      false
    end
  end
end

ReplaceWithPackagedRooftopHeatPumpHvac.new.registerWithApplication
