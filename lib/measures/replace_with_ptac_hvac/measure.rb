class ReplaceWithPtacHvac < OpenStudio::Measure::ModelMeasure
  def name
    'Replace HVAC with PTAC'
  end

  def description
    'Removes HVAC dedicated to selected zones and installs packaged terminal air conditioners.'
  end

  def modeler_description
    'Rejects partial removal of shared air loops, preserves unrelated HVAC and plant loops, then creates PTAC equipment with openstudio-standards.'
  end
  ...
  def arguments(_model)
    ...
    standards_template.setDescription('openstudio-standards template used to construct the replacement PTAC system.')
  ...
  def run(model, runner, user_arguments)
    ...
    begin
      ...
      unless missing_zones.empty?
        runner.registerError("PTAC replacement did not produce equipment for: #{missing_zones.map { |zone| zone.name.to_s }.join(', ')}")
        return false
      end

      runner.registerFinalCondition(
        "Removed #{affected_air_loops.size} dedicated air loops and #{removed_zone_equipment} zone HVAC objects; " \
        "created #{created.size} PTAC units for #{zones.size} zones. Existing plant loops were preserved."
      )
      true
    rescue LoadError => e
      ...
      runner.registerError("PTAC replacement failed: #{e.message}")
      false
    end
  end
end

ReplaceWithPtacHvac.new.registerWithApplication
