class CreatePackagedRooftopAcHvac < OpenStudio::Measure::ModelMeasure
  def name
    'Create Packaged Rooftop AC HVAC'
  end

  def description
    'Adds packaged single-zone rooftop air-conditioning systems to unserved target zones.'
  end

  def modeler_description
    'Uses the openstudio-standards PSZ-AC system and verifies that every selected zone is served by a newly created air loop.'
  end
  ...
  def arguments(_model)
    ...
    template.setDescription('openstudio-standards template used to construct the PSZ-AC system.')
  ...
  def run(model, runner, user_arguments)
    ...
    unless conflicts.empty?
      runner.registerError("PSZ-AC creation requires unserved zones. Conflicts: #{conflicts.map { |zone| zone.name.to_s }.join(', ')}")
      return false
    end
    ...
    begin
      ...
      unless missing_zones.empty?
        runner.registerError("PSZ-AC creation did not serve: #{missing_zones.map { |zone| zone.name.to_s }.join(', ')}")
        return false
      end
      runner.registerFinalCondition("Created #{created.size} PSZ-AC air loops for #{zones.size} zones using #{template}.")
      true
    rescue LoadError => e
      runner.registerError("openstudio-standards is unavailable: #{e.message}")
      false
    rescue StandardError => e
      runner.registerError("PSZ-AC creation failed: #{e.message}")
      false
    end
  end
end

CreatePackagedRooftopAcHvac.new.registerWithApplication
