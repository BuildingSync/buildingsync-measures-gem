class HvacVavHwReheat < OpenStudio::Measure::ModelMeasure
  def name = 'VAV with Hot Water Reheat HVAC'
  def description = 'Creates built-up VAV with hot-water reheat or replaces HVAC dedicated to selected zones.'
  def modeler_description = 'Uses one VAV Reheat synthesis path; replacement rejects partial shared-loop removal and preserves pre-existing plants.'
  ...
  def run(model, runner, user_arguments)
    ...
    begin
      ...
      unless unserved.empty?
        runner.registerError("VAV Reheat #{operation} did not serve: #{unserved.map { |zone| zone.name.to_s }.join(', ')}")
        return false
      end
      summary = "Created #{loops.size} VAV Reheat loops for #{zones.size} zones using #{template}."
      ...
      runner.registerError("VAV Reheat #{operation} failed: #{e.message}")
      false
    end
  end
end

HvacVavHwReheat.new.registerWithApplication