class HvacWaterLoopHeatPump < OpenStudio::Measure::ModelMeasure
  def name = 'Water-Loop Heat Pump HVAC'
  def description = 'Creates water-loop heat pumps in unserved zones or replaces HVAC dedicated to selected zones.'
  def modeler_description = 'Uses one Water Source Heat Pumps synthesis path; replacement rejects partial shared-loop removal and preserves pre-existing plants.'
  ...
  def run(model, runner, user_arguments)
    ...
    begin
      ...
      unless unserved.empty?
        runner.registerError("Water-loop heat-pump #{operation} did not serve: #{unserved.map { |zone| zone.name.to_s }.join(', ')}")
        return false
      end
      summary = "Created #{units.size} water-to-air heat pumps and #{plants.size} plant loops for #{zones.size} zones using #{template}."
      ...
      runner.registerError("Water-loop heat-pump #{operation} failed: #{e.message}")
      false
    end
  end
end

HvacWaterLoopHeatPump.new.registerWithApplication