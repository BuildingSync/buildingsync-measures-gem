class HvacPthp < OpenStudio::Measure::ModelMeasure
  def name = 'PTHP HVAC'
  def description = 'Creates PTHP equipment in unserved zones or replaces HVAC dedicated to selected zones.'
  def modeler_description = 'Uses one synthesis path for both operations; replacement rejects partial shared-air-loop removal and preserves plant loops.'
  ...
  def arguments(_model)
    ...
    operation.setDescription('Create requires unserved zones. Replace removes HVAC dedicated to an explicit zone scope.')
  ...
  def run(model, runner, user_arguments)
    ...
    if operation == 'create'
      ...
      unless conflicts.empty?
        runner.registerError("PTHP creation requires unserved zones. Conflicts: #{conflicts.map { |zone| zone.name.to_s }.join(', ')}")
        return false
      end
    else
    ...
    begin
      ...
      before = model.getZoneHVACPackagedTerminalHeatPumps.map { |unit| unit.handle.to_s }
      template = runner.getStringArgumentValue('standards_template', user_arguments)
      Standard.build(template).model_add_hvac_system(model, 'PTHP', 'NaturalGas', nil, 'Electricity', zones)
      created = model.getZoneHVACPackagedTerminalHeatPumps.reject { |unit| before.include?(unit.handle.to_s) }
      served = created.filter_map { |unit| unit.thermalZone.get if unit.thermalZone.is_initialized }
      unserved = zones - served
      unless unserved.empty?
        runner.registerError("PTHP #{operation} did not produce equipment for: #{unserved.map { |zone| zone.name.to_s }.join(', ')}")
        return false
      end
      summary = "Created #{created.size} PTHP units for #{zones.size} zones using #{template}."
      ...
      runner.registerError("PTHP #{operation} failed: #{e.message}")
      false
    end
  end
end

HvacPthp.new.registerWithApplication