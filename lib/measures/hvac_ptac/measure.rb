class HvacPtac < OpenStudio::Measure::ModelMeasure
  def name = 'PTAC HVAC'
  def description = 'Creates PTAC equipment in unserved zones or replaces HVAC dedicated to selected zones.'
  ...
  def run(model, runner, user_arguments)
    ...
    begin
      ...
      before = model.getZoneHVACPackagedTerminalAirConditioners.map { |unit| unit.handle.to_s }
      template = runner.getStringArgumentValue('standards_template', user_arguments)
      Standard.build(template).model_add_hvac_system(model, 'PTAC', 'NaturalGas', nil, 'Electricity', zones)
      created = model.getZoneHVACPackagedTerminalAirConditioners.reject { |unit| before.include?(unit.handle.to_s) }
      ...
      unless unserved.empty?
        runner.registerError("PTAC #{operation} did not produce equipment for: #{unserved.map { |zone| zone.name.to_s }.join(', ')}")
        return false
      end
      summary = "Created #{created.size} PTAC units for #{zones.size} zones using #{template}."
      ...
      runner.registerError("PTAC #{operation} failed: #{e.message}")
      false
    end
  end
end

HvacPtac.new.registerWithApplication