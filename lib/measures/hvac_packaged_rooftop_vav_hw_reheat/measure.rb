class HvacPackagedRooftopVavHwReheat < OpenStudio::Measure::ModelMeasure
  def name = 'Packaged Rooftop VAV with Hot Water Reheat HVAC'
  def description = 'Creates packaged rooftop VAV with hot-water reheat or replaces HVAC dedicated to selected zones.'
  def modeler_description = 'Uses one PVAV Reheat synthesis path; replacement rejects partial shared-loop removal and preserves pre-existing plants.'
  ...
  def run(model, runner, user_arguments)
    ...
    if operation == 'create'
      ...
      unless conflicts.empty?
        runner.registerError("PVAV Reheat creation requires unserved zones. Conflicts: #{conflicts.map { |zone| zone.name.to_s }.join(', ')}")
        return false
      end
    else
    ...
    begin
      ...
      air_handles = model.getAirLoopHVACs.map { |air_loop| air_loop.handle.to_s }
      ...
      loops = model.getAirLoopHVACs.reject { |air_loop| air_handles.include?(air_loop.handle.to_s) }
      ...
      unless unserved.empty?
        runner.registerError("PVAV Reheat #{operation} did not serve: #{unserved.map { |zone| zone.name.to_s }.join(', ')}")
        return false
      end
      summary = "Created #{loops.size} PVAV Reheat loops and #{plants.size} plant loops for #{zones.size} zones using #{template}."
      ...
      runner.registerError("PVAV Reheat #{operation} failed: #{e.message}")
      false
    end
  end
end

HvacPackagedRooftopVavHwReheat.new.registerWithApplication