class HvacPackagedRooftopHeatPump < OpenStudio::Measure::ModelMeasure
  def name = 'Packaged Rooftop Heat Pump HVAC'
  def description = 'Creates packaged single-zone rooftop heat pumps or replaces HVAC dedicated to selected zones.'
  def modeler_description = 'Uses one PSZ-HP synthesis path; replacement rejects partial shared-loop removal and preserves plant loops.'
  ...
  def run(model, runner, user_arguments)
    ...
    begin
      ...
      unless unserved.empty?
        runner.registerError("PSZ-HP #{operation} did not serve: #{unserved.map { |zone| zone.name.to_s }.join(', ')}")
        return false
      end
      summary = "Created #{created.size} PSZ-HP loops for #{zones.size} zones using #{template}."
      ...
      runner.registerError("PSZ-HP #{operation} failed: #{e.message}")
      false
    end
  end
end

HvacPackagedRooftopHeatPump.new.registerWithApplication