class CreatePthpHvac < OpenStudio::Measure::ModelMeasure
  def name
    'Create PTHP HVAC'
  end

  def description
    'Adds packaged terminal heat pumps to unserved target zones.'
  end

  def modeler_description
    'Uses openstudio-standards to create PTHP equipment. Fails when selected zones already have zone HVAC or an air-loop connection.'
  end
  ...
  def arguments(_model)
    ...
    standards_template.setDescription('openstudio-standards template used to construct the PTHP system.')
  ...
  def run(model, runner, user_arguments)
    ...
    begin
      ...
      unless missing_zones.empty?
        runner.registerError("PTHP creation did not produce equipment for: #{missing_zones.map { |zone| zone.name.to_s }.join(', ')}")
        return false
      end

      runner.registerFinalCondition("Created #{created.size} PTHP units for #{zones.size} zones using #{standards_template}.")
      true
    rescue LoadError => e
      runner.registerError("openstudio-standards is unavailable: #{e.message}")
      false
    rescue StandardError => e
      runner.registerError("PTHP creation failed: #{e.message}")
      false
    end
  end
end

CreatePthpHvac.new.registerWithApplication
