class CreateVentilationOnlyHvac < OpenStudio::Measure::ModelMeasure
  def name = 'Create Ventilation-Only HVAC'
  def description = 'Adds ventilation-only air systems to unserved target zones.'
  def modeler_description = 'Creates an outdoor-air system and supply fan on one air loop, then verifies target-zone coverage.'
  ...
  def arguments(_model)
    ...
    template.setDescription('Workflow template retained for consistent create-measure contracts; ventilation topology is constructed directly.')
  ...
  def run(model, runner, user_arguments)
    ...
    begin
      ...
      unless unserved.empty?
        runner.registerError("Ventilation-only creation did not serve: #{unserved.map { |zone| zone.name.to_s }.join(', ')}")
        return false
      end
      template = runner.getStringArgumentValue('standards_template', user_arguments)
      runner.registerFinalCondition("Created one ventilation-only loop for #{zones.size} zones using workflow template #{template}.")
      true
    rescue StandardError => e
      runner.registerError("Ventilation-only creation failed: #{e.message}")
      false
    end
  end
end

CreateVentilationOnlyHvac.new.registerWithApplication
