class CreatePackagedRooftopVavElectricReheatHvac < OpenStudio::Measure::ModelMeasure
  def name
    'Create Packaged Rooftop VAV with Electric Reheat'
  end

  def description
    'Adds packaged rooftop VAV systems with parallel fan-powered electric-reheat terminals to unserved target zones.'
  end

  def modeler_description
    'Uses the openstudio-standards PVAV PFP Boxes system and verifies that newly created air loops serve every selected zone.'
  end
  ...
  def arguments(_model)
    ...
    template.setDescription('openstudio-standards template used to construct the PVAV PFP Boxes system.')
  ...
  def run(model, runner, user_arguments)
    ...
    begin
      ...
      unless missing_zones.empty?
        runner.registerError("PVAV PFP Boxes creation did not serve: #{missing_zones.map { |zone| zone.name.to_s }.join(', ')}")
        return false
      end
      runner.registerFinalCondition("Created #{created.size} PVAV PFP Boxes loops for #{zones.size} zones using #{template}.")
      true
    rescue LoadError => e
      runner.registerError("openstudio-standards is unavailable: #{e.message}")
      false
    rescue StandardError => e
      runner.registerError("PVAV PFP Boxes creation failed: #{e.message}")
      false
    end
  end
end

CreatePackagedRooftopVavElectricReheatHvac.new.registerWithApplication
