class CreateGroundSourceHeatPumpHvac < OpenStudio::Measure::ModelMeasure
  def name = 'Create Ground-Source Heat Pump HVAC'
  def description = 'Adds ground-coupled water-to-air heat pumps and supporting loop infrastructure to unserved target zones.'
  def modeler_description = 'Uses openstudio-standards Ground Source Heat Pumps and verifies newly created zone-unit coverage.'
  ...
  def arguments(_model)
    ...
    template.setDescription('openstudio-standards template used to construct ground-source heat pumps.')
  ...
  def run(model, runner, user_arguments)
    ...
    begin
      ...
      unless unserved.empty?
        runner.registerError("Ground-source heat-pump creation did not serve: #{unserved.map { |zone| zone.name.to_s }.join(', ')}")
        return false
      end
      runner.registerFinalCondition("Created #{created.size} ground-source heat pumps and #{plants.size} plant loops for #{zones.size} zones using #{template}.")
      true
    rescue LoadError => e
      runner.registerError("openstudio-standards is unavailable: #{e.message}")
      false
    rescue StandardError => e
      runner.registerError("Ground-source heat-pump creation failed: #{e.message}")
      false
    end
  end
end

CreateGroundSourceHeatPumpHvac.new.registerWithApplication
