class ReplaceWithVavElectricReheatHvac < OpenStudio::Measure::ModelMeasure
  def name = 'Replace HVAC with VAV and Electric Reheat'
  def description = 'Replaces HVAC dedicated to selected zones with built-up VAV and electric-reheat terminals.'
  def modeler_description = 'Rejects partial shared-loop removal, preserves existing plants, and creates VAV PFP Boxes.'
  ...
  def run(model, runner, user_arguments)
    ...
    begin
      ...
      unless unserved.empty?
        runner.registerError("VAV PFP Boxes replacement did not serve: #{unserved.map { |zone| zone.name.to_s }.join(', ')}")
        return false
      end
      runner.registerFinalCondition("Removed #{affected.size} loops and #{removed} zone objects; created #{created.size} VAV PFP Boxes loops and preserved #{preserved_plants} plants.")
      true
    rescue LoadError => e
      runner.registerError("openstudio-standards is unavailable: #{e.message}")
      false
    rescue StandardError => e
      runner.registerError("VAV PFP Boxes replacement failed: #{e.message}")
      false
    end
  end
end

ReplaceWithVavElectricReheatHvac.new.registerWithApplication
