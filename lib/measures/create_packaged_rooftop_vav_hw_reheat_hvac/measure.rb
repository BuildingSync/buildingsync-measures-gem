class CreatePackagedRooftopVavHwReheatHvac < OpenStudio::Measure::ModelMeasure
  def name
    'Create Packaged Rooftop VAV with Hot Water Reheat'
  end

  def description
    'Adds packaged rooftop VAV systems with hot-water terminal reheat to unserved target zones.'
  end

  def modeler_description
    'Uses the openstudio-standards PVAV Reheat system and verifies that newly created air loops serve every selected zone.'
  end
  ...
  def arguments(_model)
    ...
    template.setDescription('openstudio-standards template used to construct the PVAV Reheat system.')
  ...
  def run(model, runner, user_arguments)
    ...
    begin
      ...
      unless missing_zones.empty?
        runner.registerError("PVAV Reheat creation did not serve: #{missing_zones.map { |zone| zone.name.to_s }.join(', ')}")
        return false
      end
      runner.registerFinalCondition("Created #{created_loops.size} PVAV Reheat loops and #{created_plants.size} plant loops for #{zones.size} zones using #{template}.")
      true
    rescue LoadError => e
      runner.registerError("openstudio-standards is unavailable: #{e.message}")
      false
    rescue StandardError => e
      runner.registerError("PVAV Reheat creation failed: #{e.message}")
      false
    end
  end
end

CreatePackagedRooftopVavHwReheatHvac.new.registerWithApplication
